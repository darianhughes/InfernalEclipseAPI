...
{
    public class IPiecewiseRotation
    {
        /// <summary>
        ...
        public IPiecewiseRotation Add(EasingCurve curve, EasingType curveType, Quaternion endingRotation, float animationEnd, Quaternion? startingRotation = null)
        {
        ...
        public Quaternion Evaluate(float interpolant, bool takeOptimalRoute, int inversionDirection)
        {
            ...
            interpolant = InfernalUtilities.Saturate(interpolant);
            ...
            float curveLocalInterpolant = InfernalUtilities.InverseLerp(segmentToUse.AnimationStart, segmentToUse.AnimationEnd, interpolant);
            ...
            if (similarity.INonZeroSign() != inversionDirection && takeOptimalRoute)
            {
                similarity *= -1f;
                start *= -1f;
            }

            float angle = MathF.Acos(Clamp(similarity, -0.9999f, 0.9999f));
            float cosecantAngle = 1f / MathF.Sin(angle);
            return (start * MathF.Sin((1f - segmentInterpolant) * angle) + end * MathF.Sin(segmentInterpolant * angle)) * cosecantAngle;
        }
    }
}
