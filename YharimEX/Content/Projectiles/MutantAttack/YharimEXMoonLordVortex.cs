                ...
                if (Projectile.localAI[1] == 0)
                {
                    if (YharimEXGlobalUtilities.HostCheck)
                    {
                        for (int i = -1; i <= 1; i += 2)
                        {
                            ...
                            Projectile.NewProjectile(Terraria.Entity.InheritSource(Projectile), Projectile.Center, vel, ModContent.ProjectileType<YharimEXGlowLine>(), 0, 0f, Main.myPlayer, 14f, Projectile.identity);
                        }
                    }
                }

                /*    Projectile ritual = YharimEXGlobalUtilities.ProjectileExists(Projectile.localAI[0], ModContent.ProjectileType<LunarRitual>());
                    if (ritual == null)
                    {
