...
using InfernalEclipseAPI.YharimEX.Content.BossBars;
    ...
    public class YharimEXBoss : ModNPC
    {
        ...
        public override bool CanHitPlayer(Player target, ref int CooldownSlot)
        {
            ...
            if (!(WorldSaveSystem.InfernumModeEnabled))
                return false;
            return NPC.Distance(YharimEXUtils.ClosestPointInHitbox(target, NPC.Center)) < Player.defaultHeight && AttackChoice > -1;
        }
        ...
        public override void AI()
        {
            ...
            if (!(WorldSaveSystem.InfernumModeEnabled) || AttackChoice != 5 && AttackChoice != 6) //spear dash direct p1
            {
                AuraCenter = Vector2.Lerp(AuraCenter, NPC.Center, 0.3f);
            }
            //in emode p2
            if ((CalamityWorld.death) && (AttackChoice < 0 || AttackChoice > 10 || AttackChoice == 10 && NPC.ai[1] > 150))
            {
                Main.dayTime = false;
        ...
        void ManageAurasAndPreSpawn()
        {
            ...
            if ((WorldSaveSystem.InfernumModeEnabled) && Main.LocalPlayer.active && !Main.LocalPlayer.dead && !Main.LocalPlayer.ghost)
            ...
            if (NPC.localAI[3] == 0)
            {
                ...
                if (NPC.Distance(Main.player[NPC.target].Center) < 1500)
                {
                    ...
                    if (YharimEXWorldFlags.AngryYharimEX && (WorldSaveSystem.InfernumModeEnabled))
            ...
            else
            {
                if (Main.LocalPlayer.active && NPC.Distance(Main.LocalPlayer.Center) < 3000f)
                {
                    ...
                    if ((CalamityWorld.death) && AttackChoice < 0 && AttackChoice > -6)
                    {
                        if (WorldSaveSystem.InfernumModeEnabled)
                        {
        ...
        {
            if (YharimEXUtils.HostCheck) //checks for needed projs
            {
                if ((CalamityWorld.death) && AttackChoice != -7 && (AttackChoice < 0 || AttackChoice > 10) && YharimEXUtils.ProjectileExists(ritualProj, ModContent.ProjectileType<YharimEXRitual>()) == null)
                    ritualProj = Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, Vector2.Zero, ModContent.ProjectileType<YharimEXRitual>(), YharimEXUtils.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer, 0f, NPC.whoAmI);

                if (YharimEXUtils.ProjectileExists(ringProj, ModContent.ProjectileType<YharimEXRitual5>()) == null)
                ...
                if (YharimEXUtils.ProjectileExists(spriteProj, ModContent.ProjectileType<YharimEXBossProjectile>()) == null)
                {
                    /*if (Main.netMode == NetmodeID.Server)
        ...
        void ChooseNextAttack(params int[] args)
        {
            ...
            if (CalamityWorld.death)
            {
                ...
                bool useRandomizer = NPC.localAI[3] >= 3 && (WorldSaveSystem.InfernumModeEnabled || Main.rand.NextFloat(0.8f) + 0.2f > (float)Math.Pow((float)NPC.life / NPC.lifeMax, 2));
            ...
            if (YharimEXUtils.HostCheck)
            {
                int maxMemory = WorldSaveSystem.InfernumModeEnabled ? 12 : 18;
            ...
            endTimeVariance = WorldSaveSystem.InfernumModeEnabled ? Main.rand.NextFloat(-0.5f, 1f) : 0;
        ...
        {
            if ((WorldSaveSystem.InfernumModeEnabled) && Main.rand.NextBool(3))
            {
            ...
            if (forceDespawn || (!p.active || p.dead || Vector2.Distance(NPC.Center, p.Center) > 3000f) && NPC.localAI[3] > 0)
            {
                ...
                if (forceDespawn || !p.active || p.dead || Vector2.Distance(NPC.Center, p.Center) > 3000f)
                {
                    ...
                    if (NPC.timeLeft == 1)
                    {
                        ...
                        if (NPC.position.Y < 0)
                            NPC.position.Y = 0;
                        if (YharimEXUtils.HostCheck && !NPC.AnyNPCs(ModContent.NPCType<TheGodseeker>()) && YharimEXWorldFlags.DownedYharimEX)
                        {
                            YharimEXUtils.ClearHostileProjectiles(2, NPC.whoAmI);
        ...
        bool Phase2Check()
        {
            if (Main.expertMode && NPC.life < NPC.lifeMax * (2f / 3))
            {
                if (YharimEXUtils.HostCheck)
                {
                    ...
                    NPC.ai[3] = 0;
                    NPC.netUpdate = true;
                    // NOTE                YharimEXUtils.ClearHostileProjectiles(1, NPC.whoAmI);
        ...
        private void EdgyBossText(string text)
        {
            if (Main.zenithWorld) //edgy boss text
            {
                ...
                YharimEXUtils.PrintText(text, color);
        ...
        void SpearTossDirectP1AndChecks()
        {
            ...
            if (NPC.ai[3] == 0)
            {
                NPC.ai[3] = WorldSaveSystem.InfernumModeEnabled ? Main.rand.Next(2, 8) : 5;
                NPC.netUpdate = true;
            }
            ...
            if (NPC.ai[1] > 150) //120)
            {
                ...
                else if (YharimEXUtils.HostCheck)
                {
                    ...
                    Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, vel, ModContent.ProjectileType<YharimEXSpearThrown>(), YharimEXUtils.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer, NPC.target);
                    if (WorldSaveSystem.InfernumModeEnabled)
                    {
                        Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, Vector2.Normalize(vel), ModContent.ProjectileType<YharimEXDeathray2>(), YharimEXUtils.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer);
                        Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, -Vector2.Normalize(vel), ModContent.ProjectileType<YharimEXDeathray2>(), YharimEXUtils.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer);
                    }
                }
                NPC.localAI[0] = 0;
            }
            else if (NPC.ai[1] == 61 && NPC.ai[2] < NPC.ai[3] && YharimEXUtils.HostCheck)
            {
                if ((CalamityWorld.death) && YharimEXWorldFlags.SkipYharimEXP1 >= 10 && !(WorldSaveSystem.InfernumModeEnabled))
                {
                    ...
                    if (YharimEXWorldFlags.SkipYharimEXP1 == 10)
                        YharimEXUtils.PrintLocalization($"Mods.{Mod.Name}.NPCs.SkipPhase1", Color.OrangeRed);
                ...
                if ((WorldSaveSystem.InfernumModeEnabled) && NPC.ai[2] == 0) //first time only
                {
                    SoundEngine.PlaySound(SoundID.NPCDeath13, NPC.Center);
                    if (YharimEXUtils.HostCheck) //spawn worm
                    {
                        ...
                        for (int j = 0; j < 8; j++)
                        {
                            ...
                            int current = Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, vel, ModContent.ProjectileType<YharimEXDestroyerHead>(), YharimEXUtils.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer, NPC.target, ai1, appearance);
                            ...
                            for (int i = 0; i < max; i++)
                                current = Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, vel, ModContent.ProjectileType<YharimEXDestroyerBody>(), YharimEXUtils.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer, Main.projectile[current].identity, 0f, appearance);
                            int previous = current;
                            current = Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, vel, ModContent.ProjectileType<YharimEXDestroyerTail>(), YharimEXUtils.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer, Main.projectile[current].identity, 0f, appearance);
                ...
                Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, Vector2.Zero, ModContent.ProjectileType<YharimEXSpearAim>(), YharimEXUtils.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer, NPC.whoAmI, 3);

                //Projectile.NewProjectile(npc.GetSource_FromThis(), NPC.Center, Vector2.Zero, ModContent.ProjectileType<YharimEXSpearAim>(), YharimEXUtils.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer, NPC.whoAmI);
            }
        }
        ...
        void OkuuSpheresP1()
        {
            ...
            {
                NPC.netUpdate = true;
                float modifier = WorldSaveSystem.InfernumModeEnabled ? 3 : 1;
                NPC.ai[1] = 90 / modifier;
                if (++NPC.ai[2] > 4 * modifier)
                {
                    if (!(WorldSaveSystem.InfernumModeEnabled) || NPC.ai[2] > 6 * modifier) //extra endtime in maso
                    {
                        P1NextAttackOrMasoOptions(AttackChoice);
                    }
                ...
                else
                {
                    ...
                    int max = WorldSaveSystem.InfernumModeEnabled ? 9 : 6;
                    float speed = WorldSaveSystem.InfernumModeEnabled ? 10 : 9;
                    int sign = WorldSaveSystem.InfernumModeEnabled ? NPC.ai[2] % 2 == 0 ? 1 : -1 : 1;
                    SpawnSphereRing(max, speed, (int)(0.8 * YharimEXUtils.ScaledProjectileDamage(NPC.defDamage)), 1f * sign);
                    SpawnSphereRing(max, speed, (int)(0.8 * YharimEXUtils.ScaledProjectileDamage(NPC.defDamage)), -0.5f * sign);
                }
        ...
        void TrueEyeDive()
        {
            ...
            {
                NPC.ai[1] = 15;
                int maxEyeThreshold = WorldSaveSystem.InfernumModeEnabled ? 6 : 3;
                int endlag = WorldSaveSystem.InfernumModeEnabled ? 3 : 5;
                ...
                else if (NPC.ai[2] <= maxEyeThreshold)
                {
                    if (YharimEXUtils.HostCheck)
                    {
                        ...
                        int p = Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, Vector2.Zero, type, YharimEXUtils.ScaledProjectileDamage(NPC.defDamage, 0.8f), 0f, Main.myPlayer, NPC.target);
        ...
        void PrepareSpearDashDirectP1()
        {
            ...
            if (NPC.ai[3] == 0)
            {
                ...
                if (YharimEXUtils.HostCheck)
                {
                    Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, Vector2.Zero, ModContent.ProjectileType<YharimEXSpearSpin>(), YharimEXUtils.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer, NPC.whoAmI, 240); // 250);
        ...
        void SpearDashDirectP1()
        {
            ...
            if (NPC.ai[3] == 0)
                NPC.ai[3] = WorldSaveSystem.InfernumModeEnabled ? Main.rand.Next(3, 15) : 10;
            ...
            if (++NPC.ai[1] > NPC.ai[3])
            {
                ...
                {
                    float speed = WorldSaveSystem.InfernumModeEnabled ? 45f : 30f;
                    NPC.velocity = speed * NPC.SafeDirectionTo(player.Center + player.velocity);
                    if (YharimEXUtils.HostCheck)
                    {
                        Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, Vector2.Zero, ModContent.ProjectileType<YharimEXSpearDash>(), YharimEXUtils.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer, NPC.whoAmI);

                        if (WorldSaveSystem.InfernumModeEnabled)
                        {
                            Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, Vector2.Normalize(NPC.velocity), ModContent.ProjectileType<YharimEXDeathray2>(), YharimEXUtils.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer);
                            Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, -Vector2.Normalize(NPC.velocity), ModContent.ProjectileType<YharimEXDeathray2>(), YharimEXUtils.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer);
                        }
                    }
        ...
        void VoidRaysP1()
        {
            ...
            {
                if (YharimEXUtils.HostCheck)
                    Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, new Vector2(2, 0).RotatedBy(NPC.ai[2]), ModContent.ProjectileType<YharimEXMark1>(), YharimEXUtils.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer);
                NPC.ai[1] = WorldSaveSystem.InfernumModeEnabled ? 3 : 5; //delay between projs
                NPC.ai[2] += NPC.ai[3];
                if (NPC.localAI[0]++ == 20 || NPC.localAI[0] == 40)
                {
                    NPC.netUpdate = true;
                    NPC.ai[2] -= NPC.ai[3] / (WorldSaveSystem.InfernumModeEnabled ? 3 : 2);

                    EdgyBossText(GFBQuote(6));
                }
                else if (NPC.localAI[0] >= (WorldSaveSystem.InfernumModeEnabled ? 60 : 40))
                {
        ...
        void BoundaryBulletHellAndSwordP1()
        {
            switch ((int)NPC.localAI[2])
            {
                case 0: //boundary lite
                    ...
                    //if ((WorldSaveSystem.InfernumModeEnabled) && NPC.ai[3] >= 300) //spear barrage
                    //{
                    //    if (NPC.ai[3] == 0)
                    //    {
                    //        NPC.ai[1] = 0;
                    ...
                    if (++NPC.ai[1] > 2) //boundary
                    {
                        ...
                        NPC.ai[2] += WorldSaveSystem.InfernumModeEnabled //maso uses true boundary
                                ? (float)Math.PI / 8 / 480 * (NPC.ai[3] - 300) * NPC.localAI[0]
                                : MathHelper.Pi / 77f;

                        if (YharimEXUtils.HostCheck)
                        {
                            int max = WorldSaveSystem.InfernumModeEnabled ? 5 : 4;
                            for (int i = 0; i < max; i++)
                            {
                                Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, new Vector2(0f, -7f).RotatedBy(NPC.ai[2] + MathHelper.TwoPi / max * i),
                                    ModContent.ProjectileType<YharimEXEye>(), YharimEXUtils.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer);
                            }
                        }

                    }

                    if (++NPC.ai[3] > (WorldSaveSystem.InfernumModeEnabled ? 360 : 240))
                    {
                        P1NextAttackOrMasoOptions(AttackChoice);
                    }
                    break;
        ...
        void PrepareYharimEXSword()
        {
            ...
            if (NPC.ai[2] == 0) //move onscreen so player can see
            {
                ...
                if ((++NPC.localAI[0] > 30 || (WorldSaveSystem.InfernumModeEnabled)) && NPC.Distance(targetPos) < 64)
                {
                    NPC.velocity = Vector2.Zero;
                    NPC.netUpdate = true;
                    ...
                    if (YharimEXUtils.HostCheck)
                    {
                        ...
                        {
                            Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center + pos, Vector2.Zero, ModContent.ProjectileType<YharimEXSword>(), YharimEXUtils.ScaledProjectileDamage(NPC.defDamage, 4f / 3f), 0f, Main.myPlayer, NPC.whoAmI, spacing);
                        }
        ...
        void YharimEXSword()
        {
            ...
            if (NPC.ai[1] == 20)
            {
                ...
                int explosions = 0;
                if ((CalamityWorld.death) && AttackChoice != 9 || WorldSaveSystem.InfernumModeEnabled)
                    explosions = 8;
                    ...
                    for (int i = 0; i < max; i++)
                    {
                        ...
                        if (YharimEXUtils.HostCheck)
                        {
                            Projectile.NewProjectile(NPC.GetSource_FromThis(), spawnPos + Main.rand.NextVector2Circular(NPC.width / 2, NPC.height / 2), Vector2.Zero, ModContent.ProjectileType<YharimEXSunBlast>(),
                                YharimEXUtils.ScaledProjectileDamage(NPC.defDamage, 4f / 3f), 0f, Main.myPlayer, MathHelper.WrapAngle(angle.ToRotation()), ai1);
                        }
                    }
                }
            }
            ...
            if (++NPC.ai[1] > 25)
            {
                ...
                else if ((WorldSaveSystem.InfernumModeEnabled) && NPC.localAI[2] < 3 * (endTimeVariance + 0.5))
                {
                    AttackChoice--;
                    NPC.ai[1] = 0;
                    NPC.ai[2] = 0;
        ...
        void Phase2Transition()
        {
            ...
            if (NPC.ai[1] == 0)
            {
                // NOTE        YharimEXUtils.ClearAllProjectiles(2, NPC.whoAmI);
            ...
            else if (NPC.ai[1] == 150)
            {
                ...
                if ((CalamityWorld.death) && YharimEXWorldFlags.SkipYharimEXP1 <= 10)
                {
        ...
        void PrepareSpearDashPredictiveP2()
        {
            if (NPC.ai[3] == 0)
            {
                ...
                if (YharimEXUtils.HostCheck)
                {
                    Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, Vector2.Zero, ModContent.ProjectileType<YharimEXSpearSpin>(), YharimEXUtils.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer, NPC.whoAmI, 180); // + 60);
        ...
        void SpearDashPredictiveP2()
        {
            ...
            {
                if (CalamityWorld.death)
                    NPC.localAI[1] = Main.rand.Next(WorldSaveSystem.InfernumModeEnabled ? 3 : 5, 9);
                else
                    NPC.localAI[1] = 5;
            }
            ...
            if (NPC.ai[1] == 0) //telegraph
            {
                ...
                if (NPC.ai[2] < NPC.localAI[1])
                {
                    ...
                    if (NPC.ai[2] == NPC.localAI[1] - 1)
                    {
                        ...
                        if (YharimEXUtils.HostCheck)
                            Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, Vector2.Zero, ModContent.ProjectileType<YharimEXSpearAim>(), YharimEXUtils.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer, NPC.whoAmI, 4);
                    }
                }
            }
            ...
            if (NPC.ai[2] == NPC.localAI[1] - 1)
                endTime = 80;
            if ((WorldSaveSystem.InfernumModeEnabled) && (NPC.ai[2] == 0 || NPC.ai[2] >= NPC.localAI[1]))
                endTime = 0;
            if (++NPC.ai[1] > endTime)
            {
                NPC.netUpdate = true;
                ...
                else
                {
                    ...
                    if (YharimEXUtils.HostCheck)
                    {
                        Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, Vector2.Normalize(NPC.velocity), ModContent.ProjectileType<YharimEXDeathray2>(), YharimEXUtils.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer);
                        Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, -Vector2.Normalize(NPC.velocity), ModContent.ProjectileType<YharimEXDeathray2>(), YharimEXUtils.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer);
                        Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, Vector2.Zero, ModContent.ProjectileType<YharimEXSpearDash>(), YharimEXUtils.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer, NPC.whoAmI, spearAi);
                    }
        ...
        void BoundaryBulletHellP2()
        {
            ...
            if (++NPC.ai[3] > endTime)
            {
                ChooseNextAttack(11, 13, 19, 20, 21, 24, WorldSaveSystem.InfernumModeEnabled ? 31 : 26, 33, 41, 44);
            }
        }
        ...
        void PillarDunk()
        {
            ...
            if (NPC.ai[2] == 0 && NPC.ai[3] == 0) //target one corner of arena
            {
                ...
                if (YharimEXUtils.HostCheck)
                {
                    void Clone(float ai1, float ai2, float ai3) => YharimEXUtils.NewNPCEasy(NPC.GetSource_FromAI(), NPC.Center, ModContent.NPCType<YharimEXIllusion>(), NPC.whoAmI, NPC.whoAmI, ai1, ai2, ai3);
            ...
            else if (NPC.ai[1] == pillarAttackDelay)
            {
                if (YharimEXUtils.HostCheck)
                {
                    Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, Vector2.UnitY * -5,
                        ModContent.ProjectileType<YharimEXPillar>(), YharimEXUtils.ScaledProjectileDamage(NPC.defDamage, 4f / 3f), 0, Main.myPlayer, 3, NPC.whoAmI);
                }
            }
            else if ((WorldSaveSystem.InfernumModeEnabled) && NPC.ai[1] == pillarAttackDelay * 5)
            {
                if (YharimEXUtils.HostCheck)
                {
                    Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, Vector2.UnitY * -5,
                        ModContent.ProjectileType<YharimEXPillar>(), YharimEXUtils.ScaledProjectileDamage(NPC.defDamage, 4f / 3f), 0, Main.myPlayer, 1, NPC.whoAmI);
                }
            }
        }
        ...
        void EOCStarSickles()
        {
            ...
            if (NPC.ai[1] == 0)
            {
                ...
                if (WorldSaveSystem.InfernumModeEnabled) //begin attack much faster
                {
                    ai1 = 30;
                    NPC.ai[1] = 30;
                }
                ...
                if (YharimEXUtils.HostCheck)
                {
                    int p = Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, -Vector2.UnitY, ModContent.ProjectileType<YharimEXEyeOfCthulhu>(), YharimEXUtils.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer, NPC.target, ai1);
                    if ((WorldSaveSystem.InfernumModeEnabled) && p != Main.maxProjectiles)
            ...
            if (--NPC.ai[1] < 0)
            {
                ...
                else
                {
                    if (YharimEXUtils.HostCheck)
                        for (int i = 0; i < 8; i++)
                            Projectile.NewProjectile(npc.GetSource_FromThis(), NPC.Center, Vector2.UnitX.RotatedBy(Math.PI / 4 * i) * 10f, ModContent.ProjectileType<YharimEXScythe1>(), YharimEXUtils.ScaledProjectileDamage(NPC.defDamage, 0.8f), 0f, Main.myPlayer, NPC.whoAmI);
        ...
        void PrepareSpearDashDirectP2()
        {
            if (NPC.ai[3] == 0)
            {
                ...
                if (YharimEXUtils.HostCheck)
                {
                    Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, Vector2.Zero, ModContent.ProjectileType<YharimEXSpearSpin>(), YharimEXUtils.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer, NPC.whoAmI, 180);// + (WorldSavingSystem.MasochistMode ? 10 : 20));
                    TelegraphSound = SoundEngine.PlaySound(YharimEXSoundRegistry.YharimEXUnpredictive with { Volume = 2f }, NPC.Center);
                }
        ...
        void SpearDashDirectP2()
        {
            ...
            if (++NPC.ai[1] > (CalamityWorld.death ? 5 : 20))
            {
                NPC.netUpdate = true;
                AttackChoice++;
                NPC.ai[1] = 0;
                ...
                {
                    NPC.velocity = NPC.SafeDirectionTo(player.Center) * (WorldSaveSystem.InfernumModeEnabled ? 60f : 45f);
                    if (YharimEXUtils.HostCheck)
                    {
                        Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, Vector2.Normalize(NPC.velocity), ModContent.ProjectileType<YharimEXDeathray2>(), YharimEXUtils.ScaledProjectileDamage(NPC.defDamage, 0.8f), 0f, Main.myPlayer);
                        Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, -Vector2.Normalize(NPC.velocity), ModContent.ProjectileType<YharimEXDeathray2>(), YharimEXUtils.ScaledProjectileDamage(NPC.defDamage, 0.8f), 0f, Main.myPlayer);
                        Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, Vector2.Zero, ModContent.ProjectileType<YharimEXSpearDash>(), YharimEXUtils.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer, NPC.whoAmI);
                    }
                }
        ...
        void SpearTossPredictiveP2()
        {
            ...
            if (++NPC.ai[1] > 60)
            {
                ...
                if ((shouldAttack || (WorldSaveSystem.InfernumModeEnabled)) && YharimEXUtils.HostCheck)
                {
                    Vector2 vel = NPC.SafeDirectionTo(player.Center + player.velocity * 30f) * 30f;
                    Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, Vector2.Normalize(vel), ModContent.ProjectileType<YharimEXDeathray2>(), YharimEXUtils.ScaledProjectileDamage(NPC.defDamage, 0.8f), 0f, Main.myPlayer);
                    Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, -Vector2.Normalize(vel), ModContent.ProjectileType<YharimEXDeathray2>(), YharimEXUtils.ScaledProjectileDamage(NPC.defDamage, 0.8f), 0f, Main.myPlayer);
                    Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, vel, ModContent.ProjectileType<YharimEXSpearThrown>(), YharimEXUtils.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer, NPC.target, 1f);
                }
            }
            else if (NPC.ai[1] == 1 && (NPC.ai[2] < NPC.localAI[1] || (WorldSaveSystem.InfernumModeEnabled)) && YharimEXUtils.HostCheck)
            {
                ...
                Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, Vector2.Zero, ModContent.ProjectileType<YharimEXSpearAim>(), YharimEXUtils.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer, NPC.whoAmI, 2);
            }
        }
        ...
        void PrepareMechRayFan()
        {
            ...
            if (++NPC.ai[1] > 150 || (WorldSaveSystem.InfernumModeEnabled) && NPC.Distance(targetPos) < 64)
            {
                NPC.netUpdate = true;
                AttackChoice++;
                NPC.ai[1] = 0;
        ...
        void MechRayFan()
        {
            ...
            if (NPC.ai[3] == 0 && YharimEXUtils.HostCheck)
            {
                int max = 7;
                for (int i = 0; i <= max; i++)
                {
            ...
            if (NPC.ai[3] > (WorldSaveSystem.InfernumModeEnabled ? 45 : 60) && NPC.ai[3] < 60 + 180 && ++NPC.ai[1] > 10)
            {
                NPC.ai[1] = 0;
                if (YharimEXUtils.HostCheck)
                {
                    ...
                    void SpawnRay(Vector2 pos, float angleInDegrees, float turnRotation)
                    {
                        int p = Projectile.NewProjectile(NPC.GetSource_FromThis(), pos, MathHelper.ToRadians(angleInDegrees).ToRotationVector2(),
                            ModContent.ProjectileType<YharimEXDeathray3>(), YharimEXUtils.ScaledProjectileDamage(NPC.defDamage), 0, Main.myPlayer, turnRotation, NPC.whoAmI);
            ...
            void SpawnPrime(float varianceInDegrees, float rotationInDegrees)
            {
                ...
                if (YharimEXUtils.HostCheck)
                {
                    ...
                    Projectile.NewProjectile(NPC.GetSource_FromThis(), spawnPos, vel, ModContent.ProjectileType<YharimEXGuardian>(), YharimEXUtils.ScaledProjectileDamage(NPC.defDamage, 4f / 3f), 0f, Main.myPlayer);
                }
            }
            ...
            //if ((WorldSaveSystem.InfernumModeEnabled) && NPC.ai[3] == endTime - 40)
            //{
            //    Vector2 aimPoint = NPC.Center - Vector2.UnitY * NPC.ai[2] * 600;
            //    for (int i = -3; i <= 3; i++)
            //    {
            ...
            //        if (YharimEXUtils.HostCheck)
            //            Projectile.NewProjectile(NPC.GetSource_FromThis(), spawnPos, Vector2.Zero, ModContent.ProjectileType<YharimEXReticle2>(), YharimEXUtils.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer);
            //    }
            //}

            if (++NPC.ai[3] > endTime)
            {
                //if ((WorldSaveSystem.InfernumModeEnabled)) //maso prime jumpscare after rays
                //{
                //    for (int i = 0; i < 60; i++)
                //        SpawnPrime(45, 90);
                //}

                if (CalamityWorld.death) //use full moveset
                {
        ...
        void SpawnFishrons()
        {
            ...
            int maxFishronSets = WorldSaveSystem.InfernumModeEnabled ? 3 : 2;
                ...
                if (YharimEXUtils.HostCheck)
                {
                    ...
                    for (int j = -1; j <= 1; j += 2) //to both sides of player
                    {
                        ...
                        for (int i = -max; i <= max; i++) //fan of fishron
                        {
                            ...
                            Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, Vector2.Zero, projType, YharimEXUtils.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer, offset.X, offset.Y);
                        }
                    }
                }
                for (int i = 0; i < 30; i++)
            ...
            if (++NPC.ai[1] > (WorldSaveSystem.InfernumModeEnabled ? 60 : 120))
            {
                ChooseNextAttack(13, 19, 20, 21, WorldSaveSystem.InfernumModeEnabled ? 44 : 26, 31, 31, 31, 33, 35, 39, 41, 42, 44/*, 47*//*, 49*/);
            }
        }
        ...
        void PrepareNuke()
        {
            ...
            if (++NPC.ai[1] > 60)
            {
                ...
                {
                    float gravity = 0.2f;
                    float time = WorldSaveSystem.InfernumModeEnabled ? 120f : 180f;
                    Vector2 distance = player.Center - NPC.Center;
                    distance.X /= time;
                    distance.Y = distance.Y / time - 0.5f * gravity * time;
                    Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, distance, ModContent.ProjectileType<YharimEXNuke>(), WorldSaveSystem.InfernumModeEnabled ? YharimEXUtils.ScaledProjectileDamage(NPC.defDamage, 4f / 3f) : 0, 0f, Main.myPlayer, gravity);
                    Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, Vector2.Zero, ModContent.ProjectileType<YharimEXFishronRitual>(), YharimEXUtils.ScaledProjectileDamage(NPC.defDamage, 4f / 3f), 0f, Main.myPlayer, NPC.whoAmI);
                }
                AttackChoice++;
                NPC.ai[1] = 0;
        ...
        void Nuke()
        {
            ...
            int maxSpeed = WorldSaveSystem.InfernumModeEnabled ? 3 : 2;
            if (NPC.velocity.Length() > maxSpeed)
                NPC.velocity = Vector2.Normalize(NPC.velocity) * maxSpeed;

            if (NPC.ai[1] > (WorldSaveSystem.InfernumModeEnabled ? 120 : 180))
            {
                if (!Main.dedServ && Main.LocalPlayer.active)
                    YharimEXUtils.ScreenshakeRumble(6);
                ...
                if (YharimEXUtils.HostCheck)
                {
                    ...
                    for (int i = 0; i < 3; i++)
                    {
                        ...
                        Projectile.NewProjectile(NPC.GetSource_FromThis(), spawnPos, Vector2.Zero, ModContent.ProjectileType<YharimEXBomb>(), YharimEXUtils.ScaledProjectileDamage(NPC.defDamage, 4f / 3f), 0f, Main.myPlayer);
                    }
                }
            }
            ...
            {
                ChooseNextAttack(11, 13, 16, 19, 24, WorldSaveSystem.InfernumModeEnabled ? 26 : 29, 31, 35, 37, 39, 41, 42/*, 47*//*, 49*/);
            }
        ...
        void PrepareSlimeRain()
        {
            ...
            if (++NPC.ai[2] > 30 || (WorldSaveSystem.InfernumModeEnabled) && NPC.Distance(targetPos) < 64)
            {
                AttackChoice++;
                NPC.ai[1] = 0;
                NPC.ai[2] = 0;
        ...
        void SlimeRain()
        {
            if (NPC.ai[3] == 0)
            {
                ...
                SoundEngine.PlaySound(SoundID.Roar, NPC.Center);
                if (YharimEXUtils.HostCheck)
                    Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, Vector2.Zero, ModContent.ProjectileType<YharimEXSlimeRain>(), YharimEXUtils.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer, NPC.whoAmI);
            }
            ...
            if (NPC.ai[1] > 120 && NPC.ai[1] % 5 == 0) //rain down slime balls
            {
                ...
                if (YharimEXUtils.HostCheck)
                {
                    void Slime(Vector2 pos, float off, Vector2 vel)
                    {
                        ...
                        int flip = (WorldSaveSystem.InfernumModeEnabled) && NPC.ai[2] < 180 * 2 && Main.rand.NextBool() ? -1 : 1;
                        Vector2 spawnPos = pos + off * Vector2.UnitY * flip;
                        float ai0 = YharimEXUtils.ProjectileExists(ritualProj, ModContent.ProjectileType<YharimEXRitual>()) == null ? 0f : NPC.Distance(Main.projectile[ritualProj].Center);
                        Projectile.NewProjectile(NPC.GetSource_FromThis(), spawnPos, vel * flip * 2 /* x2 to compensate for removed extraUpdates */, ModContent.ProjectileType<YharimEXSlimeBall>(), YharimEXUtils.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer, ai0);
                    }
            ...
            if ((WorldSaveSystem.InfernumModeEnabled) && NPC.ai[1] == 120 && NPC.ai[2] < masoMovingRainAttackTime && Main.rand.NextBool(3))
            ...
            if (++NPC.ai[2] > endTime)
            {
                ChooseNextAttack(11, 16, 19, 20, WorldSaveSystem.InfernumModeEnabled ? 26 : 29, 31, 33, 37, 39, 41, 42, 45);
            }
        }
        ...
        void QueenSlimeRain()
        {
            ...
            if (NPC.ai[1] > masoMovingRainAttackTime && NPC.ai[1] % 3 == 0) //rain down slime balls
            {
                ...
                if (YharimEXUtils.HostCheck)
                {
                    ...
                    void Slime(Vector2 pos, float off, Vector2 vel)
                    {
                        ...
                        Vector2 spawnPos = pos + off * Vector2.UnitY * flip;
                        float ai0 = YharimEXUtils.ProjectileExists(ritualProj, ModContent.ProjectileType<YharimEXRitual>()) == null ? 0f : NPC.Distance(Main.projectile[ritualProj].Center);
                        Projectile.NewProjectile(NPC.GetSource_FromThis(), spawnPos, vel * flip, ModContent.ProjectileType<YharimEXSlimeSpike>(), YharimEXUtils.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer, ai0, ai2: frame);
                    }
            ...
            {
                float safespotMoveSpeed = WorldSaveSystem.InfernumModeEnabled ? 7f : 6f;
                ...
                if (--NPC.localAI[2] < 0) //reset and recalibrate for the other direction
                {
                    ...
                    NPC.ai[2] = WorldSaveSystem.InfernumModeEnabled ? 15 : 30; //adds a pause when turning around
                }
        ...
        void PrepareFishron2()
        {
            ...
            if (++NPC.ai[1] > 60 || (WorldSaveSystem.InfernumModeEnabled) && NPC.Distance(targetPos) < 32) //dive here
            {
                NPC.velocity.X = 35f * (NPC.position.X < player.position.X ? 1 : -1);
                NPC.velocity.Y = 10f;
                AttackChoice++;
        ...
        void OkuuSpheresP2()
        {
            ...
            if (++NPC.ai[1] > 10 && NPC.ai[3] > 60 && NPC.ai[3] < endTime - 60)
            {
                ...
                int max = WorldSaveSystem.InfernumModeEnabled ? 10 : 9;
                float speed = WorldSaveSystem.InfernumModeEnabled ? 11f : 10f;
                SpawnSphereRing(max, speed, YharimEXUtils.ScaledProjectileDamage(NPC.defDamage), -1f, rotation);
                SpawnSphereRing(max, speed, YharimEXUtils.ScaledProjectileDamage(NPC.defDamage), 1f, rotation);
            ...
            if (++NPC.ai[3] > endTime)
            {
                ChooseNextAttack(13, 19, 20, WorldSaveSystem.InfernumModeEnabled ? 13 : 26, WorldSaveSystem.InfernumModeEnabled ? 44 : 33, 41, 44/*, 49*/);
            }
            ...
            if (++NPC.ai[1] > 180)
            {
                ...
                if (++NPC.ai[2] > NPC.localAI[1])
                {
                    if (Main.getGoodWorld) // Can't combo into slime rain in ftw
                        ChooseNextAttack(11, 16, 19, 20, WorldSaveSystem.InfernumModeEnabled ? 44 : 26, 31, 33, /*35,*/ 42, 44, 45/*, 47*/);
                    else
                        ChooseNextAttack(11, 16, 19, 20, WorldSaveSystem.InfernumModeEnabled ? 44 : 26, 31, 33, 35, 42, 44, 45/*, 47*/);
                    shouldAttack = false;
                }

                if (shouldAttack || (WorldSaveSystem.InfernumModeEnabled))
                {
                    SpawnSpearTossDirectP2Attack();
                }
            }
            else if ((WorldSaveSystem.InfernumModeEnabled) && NPC.ai[1] == 165)
            {
                SpawnSpearTossDirectP2Attack();
            }
            else if (NPC.ai[1] == 151)
            {
                if (NPC.ai[2] > 0 && (NPC.ai[2] < NPC.localAI[1] || (WorldSaveSystem.InfernumModeEnabled)) && YharimEXUtils.HostCheck)
                    Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, Vector2.Zero, ModContent.ProjectileType<YharimEXSpearAim>(), YharimEXUtils.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer, NPC.whoAmI, 1);
            }
            else if (NPC.ai[1] == 1)
            {
                if (YharimEXUtils.HostCheck)
        ...
        void TwinRangsAndCrystals()
        {
            ...
            if (NPC.ai[3] == 0)
            {
                ...
                if (!(WorldSaveSystem.InfernumModeEnabled) && YharimEXUtils.HostCheck)
                {
                    for (int i = 0; i < 4; i++)
                    {
                        Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center + Vector2.UnitX.RotatedBy(Math.PI / 2 * i) * 525, Vector2.Zero, ModContent.ProjectileType<YharimEXGlowRingHollow>(), YharimEXUtils.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer, 1f);
                        Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center + Vector2.UnitX.RotatedBy(Math.PI / 2 * i + Math.PI / 4) * 350, Vector2.Zero, ModContent.ProjectileType<YharimEXGlowRingHollow>(), YharimEXUtils.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer, 2f);
                    }
                }
            }

            int ringDelay = WorldSaveSystem.InfernumModeEnabled ? 12 : 15;
            int ringMax = WorldSaveSystem.InfernumModeEnabled ? 5 : 4;
            if (NPC.ai[3] % ringDelay == 0 && NPC.ai[3] < ringDelay * ringMax)
            {
                if (YharimEXUtils.HostCheck)
                {
                    ...
                    int baseDelay = 60;
                    float flyDelay = 120 + NPC.ai[3] / ringDelay * (WorldSaveSystem.InfernumModeEnabled ? 40 : 50);
                    int p = Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, 300f / baseDelay * Vector2.UnitX.RotatedBy(rotationOffset), ModContent.ProjectileType<YharimEXMark2>(), YharimEXUtils.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer, baseDelay, baseDelay + flyDelay);
                    if (p != Main.maxProjectiles)
                    {
                        ...
                        for (int i = 0; i < max; i++)
                        {
                            ...
                            Projectile.NewProjectile(NPC.GetSource_FromThis(), spawnPos, Vector2.Zero, ModContent.ProjectileType<YharimEXCrystalLeaf>(), YharimEXUtils.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer, Main.projectile[p].identity, myRot);
                        }
                    }
                }
            }
            ...
            if (NPC.ai[3] > 45 && --NPC.ai[1] < 0)
            {
                ...
                if (YharimEXUtils.HostCheck && NPC.ai[3] < 330)
                {
                    ...
                    float rotationOffset = WorldSaveSystem.InfernumModeEnabled ? MathHelper.PiOver4 : 0;
                    for (int i = 0; i < 4; i++)
                    {
                        Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, Vector2.UnitX.RotatedBy(Math.PI / 2 * i + rotationOffset) * retiSpeed, ModContent.ProjectileType<YharimEXRetirang>(), YharimEXUtils.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer, retiAcc, 300);
                        Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, Vector2.UnitX.RotatedBy(Math.PI / 2 * i + Math.PI / 4 + rotationOffset) * spazSpeed, ModContent.ProjectileType<YharimEXSpazmarang>(), YharimEXUtils.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer, spazAcc, 180);
                    }
                }
            }
            if (++NPC.ai[3] > 450)
        ...
        void EmpressSwordWave()
        {
            ...
            int attackThreshold = WorldSaveSystem.InfernumModeEnabled ? 48 : 60;
            ...
            void Sword(Vector2 pos, float ai0, float ai1, Vector2 vel)
            {
                if (YharimEXUtils.HostCheck)
                {
                    Projectile.NewProjectile(NPC.GetSource_FromThis(), pos - vel * 60f, vel,
                        ProjectileID.FairyQueenLance, YharimEXUtils.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer, ai0, ai1);
                }
            }
            ...
            if (NPC.ai[1] >= startup && NPC.ai[1] < startup + attackThreshold * timesToAttack && --NPC.ai[2] < 0) //walls of swords
            {
                ...
                int max = WorldSaveSystem.InfernumModeEnabled ? 16 : 12;
                ...
                Vector2 home = NPC.Center;// YharimEXUtils.ProjectileExists(ritualProj, ModContent.ProjectileType<YharimEXRitual>()) == null ? NPC.Center : Main.projectile[ritualProj].Center;
            ...
            void MegaSwordSwarm(Vector2 target)
            {
                ...
                for (int i = 0; i < max; i++)
                {
                    ...
                    //if ((WorldSaveSystem.InfernumModeEnabled)) //block one side so only one real exit exists
            ...
            if ((WorldSaveSystem.InfernumModeEnabled) && NPC.ai[1] == swordSwarmTime + 30)
            {
                for (int i = -1; i <= 1; i += 2)
                {
            ...
            if (++NPC.ai[1] > swordSwarmTime + (WorldSaveSystem.InfernumModeEnabled ? 60 : 30))
            {
                ChooseNextAttack(11, 13, 16, 21, WorldSaveSystem.InfernumModeEnabled ? 26 : 24, 29, 31, 35, 37, 39, 41, 45/*, 47*//*, 49*/);
            }
        }
        ...
        void SANSGOLEM()
        {
            ...
            int attackDelay = WorldSaveSystem.InfernumModeEnabled ? 50 : 70;
            ...
            if (NPC.ai[1] > 0 && NPC.ai[1] % attackDelay == 0)
            {
                ...
                Vector2 centerPoint = YharimEXUtils.ProjectileExists(ritualProj, ModContent.ProjectileType<YharimEXRitual>()) == null ? player.Center : Main.projectile[ritualProj].Center;
                ...
                for (int i = -1; i <= 1; i += 2) //left and right
                {
                    ...
                    {
                        float gapRadiusHeight = WorldSaveSystem.InfernumModeEnabled ? 120 : 150;
                        ...
                        if (YharimEXUtils.HostCheck)
                        {
                            Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, vel,
                                ...
                                YharimEXUtils.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer,
        ...
        void P2NextAttackPause() //choose next attack but actually, this also gives breathing space for mp to sync up
        {
            ...
            if (++NPC.ai[1] > 60 || NPC.Distance(targetPos) < 200 && NPC.ai[1] > (NPC.localAI[3] >= 3 ? 15 : 30))
            {
                ...
                NPC.velocity *= WorldSaveSystem.InfernumModeEnabled ? 0.25f : 0.75f;
        ...
        void VoidRaysP3()
        {
            if (--NPC.ai[1] < 0)
            {
                if (YharimEXUtils.HostCheck)
                {
                    float speed = (WorldSaveSystem.InfernumModeEnabled) && NPC.localAI[0] <= 40 ? 4f : 2f;
                    Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, speed * Vector2.UnitX.RotatedBy(NPC.ai[2]), ModContent.ProjectileType<YharimEXMark1>(), YharimEXUtils.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer);
                }
                NPC.ai[1] = 1;
                NPC.ai[2] += NPC.ai[3];
                ...
                else if (NPC.localAI[0] >= (WorldSaveSystem.InfernumModeEnabled ? 160 : 120))
                {
                    NPC.netUpdate = true;
                    AttackChoice--;
                    NPC.ai[1] = 0;
        ...
        void OkuuSpheresP3()
        {
            ...
            if (++NPC.ai[1] > 10 && NPC.ai[3] > 60 && NPC.ai[3] < endTime - 120)
            {
                ...
                int max = WorldSaveSystem.InfernumModeEnabled ? 11 : 10;
                float speed = WorldSaveSystem.InfernumModeEnabled ? 11f : 10f;
                SpawnSphereRing(max, speed, YharimEXUtils.ScaledProjectileDamage(NPC.defDamage), -0.75f, rotation);
                SpawnSphereRing(max, speed, YharimEXUtils.ScaledProjectileDamage(NPC.defDamage), 0.75f, rotation);
            }
        ...
        void BoundaryBulletHellP3()
        {
            ...
            if (++NPC.ai[1] > 3)
            {
                SoundEngine.PlaySound(SoundID.Item12, NPC.Center);
                NPC.ai[1] = 0;
                NPC.ai[2] += (float)Math.PI / 5 / 420 * NPC.ai[3] * NPC.localAI[0] * (WorldSaveSystem.InfernumModeEnabled ? 2f : 1);
                if (NPC.ai[2] > (float)Math.PI)
                    NPC.ai[2] -= (float)Math.PI * 2;
                if (YharimEXUtils.HostCheck)
                {
                    int max = WorldSaveSystem.InfernumModeEnabled ? 10 : 8;
                    for (int i = 0; i < max; i++)
                    {
        ...
        void FinalSpark()
        {
            ...
            */
            ...
            bool harderRings = (WorldSaveSystem.InfernumModeEnabled) && NPC.ai[2] >= 420 - 90;
            ...
            if (++NPC.ai[1] > ringTime)
            {
                ...
                if (YharimEXUtils.HostCheck)
                {
                    ...
                    int damage = YharimEXUtils.ScaledProjectileDamage(NPC.defDamage);
                ...
                if (NPC.localAI[1] == 0) //maso do ordinary spark
                {
                    ...
                    if (YharimEXUtils.HostCheck)
                    {
                        Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, Vector2.UnitX.RotatedBy(NPC.ai[3]),
                            ModContent.ProjectileType<YharimEXGiantDeathray2>(), YharimEXUtils.ScaledProjectileDamage(NPC.defDamage, 0.5f), 0f, Main.myPlayer, 0, NPC.whoAmI);
                    }
            ...
            else
            {
                ...
                if (NPC.ai[1] % 3 == 0 && YharimEXUtils.HostCheck)
                {
            ...
            else if (NPC.ai[2] == 420)
            {
                ...
                NPC.ai[3] += MathHelper.ToRadians(20) * (WorldSaveSystem.InfernumModeEnabled ? 1 : -1);
            ...
            SpinLaser((WorldSaveSystem.InfernumModeEnabled) && NPC.ai[2] >= 420);
        ...
        void DyingDramaticPause()
        {
            ...
            NPC.velocity = Vector2.Zero;
            bool killPlayer = WorldSaveSystem.InfernumModeEnabled;
            ...
            if (++NPC.ai[1] > 120)
            {
                ...
                NPC.netUpdate = true;
                if (YharimEXUtils.HostCheck) //shoot death anim mega ray
                {
                    int damage = WorldSaveSystem.InfernumModeEnabled ? YharimEXUtils.ScaledProjectileDamage(NPC.defDamage, 0.5f) : 0;
        ...
        void DyingAnimationAndHandling()
        {
            ...
            if (++NPC.alpha > 255)
            {
                ...
                NPC.checkDead();
                if (YharimEXUtils.HostCheck && !NPC.AnyNPCs(ModContent.NPCType<TheGodseeker>()))
                {
                    int n = NPC.NewNPC(NPC.GetSource_FromAI(), (int)NPC.Center.X, (int)NPC.Center.Y, ModContent.NPCType<TheGodseeker>());
                    if (n != Main.maxNPCs)
                    {
        ...
        public override bool CheckDead()
        {
            ...
            NPC.life = 1;
            NPC.active = true;
            if (YharimEXUtils.HostCheck && AttackChoice > -1)
            {
                AttackChoice = CalamityWorld.death ? AttackChoice >= 10 ? -1 : 10 : -6;
                NPC.ai[1] = 0;
                NPC.ai[2] = 0;
                NPC.ai[3] = 0;
                NPC.localAI[0] = 0;
                ...
                NPC.dontTakeDamage = true;
                NPC.netUpdate = true;
                YharimEXUtils.ClearAllProjectiles(2, NPC.whoAmI, AttackChoice < 0);
            }
            return false;
        }
        ...
        public override void OnKill()
        {
            ...
            //    if ((WorldSaveSystem.InfernumModeEnabled) || (!playerInvulTriggered && (CalamityWorld.death)))
            //    {
        ...
        public static void ArenaAura(Vector2 center, float distance, bool reverse = false, int dustid = -1, Color color = default, params int[] buffs)
        {
            ...
            if (p.active && !p.dead && !p.ghost && (reverse ? range > distance && range < Math.Max(3000f, distance * 2) : range < distance))
            {
                ...
                {
                    YharimEXUtils.AddDebuffFixedDuration(p, buff, 2);
                }
            }
        }
    }
